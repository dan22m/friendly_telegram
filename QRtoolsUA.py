...
class QRtoolsMod(loader.Module):
	...
	async def makeqrcmd(self, message):
		...
		if not text or text.lower() == ".file":
			...
			if not reply or not reply.message:
				await message.edit("<b>Немає тексту для кодування!</b>")
				return
			text = reply.raw_text
		else:
	...
	async def readqrcmd(self, message):
		...
		if not ok:
			...
			if not ok:
				text = "<b>Це не зображення!</b>" if reply else "<b>Нічого не передано!</b>"
		...
		if not text:
			text = "<b>Неможливо розпізнати або QR пустий!<b>"
